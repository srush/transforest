...
# this version uses closest and nist_tokenize as default.
# hacked by lhuang to output length ratio.
...
#        * 'ashortest' : act as previous versions of the script (taking shortest reference translation length)
...
sub score_system
{
	...
	foreach $doc (sort keys %eval_docs)
	{
		...
		if ( $method eq "BLEU" )
		{
            printf "length ratio: %.2f (\"%s\")\n", $tst_cnt->[ 1 ] / $ref_length, $brevity_penalty;
			my %DOCmt = ();
