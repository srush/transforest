...
def _prep_features(names, read_names=True, prep_weights=False):
	...
	bigram = [Bigram(), Bigram(htype=heads.SEM)]
	trigram = [Trigram()]
	headmod = [HeadMod(), HeadMod(htype=heads.SEM)]
	distmod = [DistMod(), DistMod(htype=heads.SEM)]

	collins = bigram + trigram + headmod + distmod
