...
modelsdir = "/home/lhuang/rerank/models"
...
class Feature(object):
	...
	def __str__(self):
		return self._str ## must be set up on __init__
...
class Rule(Feature):
	...
	def make(self, tree):
		...
		label = tree.label
		parentlabel = tree.parentlabel if hasattr(tree, "parentlabel") else None
		elements = [sub.label for sub in tree.subs] + ["_"]		
	...
	def extract(self, tree, sentence, dep=0):
		return [self.make(tree)] if not tree.is_terminal() else []
	
##		if tree.is_terminal():
##			return []
##		else:
##			if self.incl_parent == dep:
##				return [self.make(tree)]
##			else:
##				a = []
##				for sub in tree.subs:
##					sub.parentlabel = tree.label
##					a += self.extract(sub, sentence, dep+1)
##				return a
...
class RightBranch(Feature):
	...
	def make(self, a, counta):
		...
		return [("%s %d" % (self._str, a), counta)] if counta > 0 else []
	...
	##[("1", counts[1]), ("0", counts[0])] if counts[0] > 0 else [("1", counts[1])]
...
class NGramTree(Feature):
	...
	def __init__(self, ngram=2, lex=0):		
		self._str = "NGramTree:%d:%d:1:0" % (ngram, lex)
		self._locality = Feature.NONLOCAL
		self.ngram = ngram
		self.lex = lex
	...
	def out(self, path):
		...
		if self.lex == 0:
			...
			s = "(%s %s)" % (path[0].label, path[0].word)
		for node in path[1:]:
			s = "(%s %s)" % (node.label, s)
		return s
	...
	def make(self, head, leftpath, rightpath):
		...
		s = "\"(%s %s %s)\"" % (head.label, self.out(leftpath), self.out(rightpath))
		...
		if tree.is_terminal():
			...
			for i, left in enumerate(tree.subs[:-1]):
				...
				if right.span[0] != len(sentence)-1:
					a.append(self.make(tree, left.rightmost, right.leftmost))
...
def _prep_features(names, read_names=True, prep_weights=False):
	...
	heads2_nonlex = Heads(2, False, False, heads.SYN)
	heads2 = [heads2_nonlex, \
			  Heads(2, True,  True,  heads.SYN), \
			  Heads(2, True,  True,  heads.SEM)]
	heads3 =  Heads(3, False, False, heads.SYN)

	headtree = [HeadTree(False, heads.SYN), \
				HeadTree(False, heads.SEM), \
				HeadTree(True,  heads.SEM)]
	...
	implemented = {'rule-1': [r1], 'rule-2': [r2], \
				   'heavy': [h], \
	...
	for name in names:
		...
		FVector.feature_mapping = feature_mapping
		FVector.reverse_mapping = reverse_mapping
