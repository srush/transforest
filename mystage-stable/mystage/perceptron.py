...
def one_example(forest, weights, indent=" " * 5):
	...
	if math.fabs(1-loss) > 1e-4:
		...
		print >> logs, indent, "Loss = %.4lf, |delta| = %d, update_rate = %.3lf" % \
...
if __name__ == "__main__":
	...
	decoder = [NBestDecoder(opts.N), \
			   LocalDecoder(), \
			   BUDecoder(opts.k, check_feats=False)]\
			   [opts.mode]
...
##		trainforests = decoder.load(opts.trainfile)
	...
	oldtime = time.time()
	print >> logs, "starting perceptron at", oldtime
	for it in xrange(opts.iterations):
		...
		if not preloaded:
			print >> logs, "load %.2lf, oracle %.2lf" % (decoder.load_time(), decoder.oracle_time)
		else:
			print >> logs
...
# 	print >> logs, "initial f-score ======================================================"
# 	print evaluate_all(initial_weights, devforests)
##	print >> logs, "final f-score-avg ======================================================"
##	print >> logs, evaluate_all(avged_weights, devforests)

	newtime = time.time()
	print >> logs, "finishing perceptron at", newtime, " total time = %.2lf (%.2lf per iter)" % \
